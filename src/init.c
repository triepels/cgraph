...
SEXP cg_vector_abs(SEXP a1, SEXP out)
{
  if(!Rf_isNumeric(a1))
  {
    Rf_errorcall(R_NilValue, "argument 'a1' must be a numerical vector or array");
...
SEXP cg_vector_abs_grad(SEXP a1, SEXP value, SEXP grad, SEXP out)
{
  if(!Rf_isNumeric(a1))
  {
    Rf_errorcall(R_NilValue, "argument 'a1' must be a numerical vector or array");
  ...
  {
    Rf_errorcall(R_NilValue, "argument 'value' must be a numerical vector or array");
  }
  ...
  if(n != XLENGTH(value) || n != XLENGTH(grad))
  {
    Rf_errorcall(R_NilValue, "argument 'a1', 'value', and 'grad' have incompatible lengths");
  }
...
static const R_CallMethodDef CallEntries[] = {
  ...
  {"cg_vector_abs",         (DL_FUNC) &cg_vector_abs,         2},
  {"cg_vector_abs_grad",    (DL_FUNC) &cg_vector_abs_grad,    4},
  // Node
