...
SEXP cg_math_sigmoid(SEXP a1)
{
  ...
  switch(TYPEOF(a1))
  {
    case REALSXP :
    {
      ...
      {
        p0[i] = 1 / (1 + exp(-p1[i]));
        p0[i] = (p0[i] < DBL_EPSILON) ? DBL_EPSILON : p0[i];
        p0[i] = (p0[i] > 1 - DBL_EPSILON) ? 1 - DBL_EPSILON : p0[i];
      }
