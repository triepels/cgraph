...
* Function `cg_graph_forward` and `cg_graph_backward` now allow argument `target` to be the name of the target node, in which case a linear search is performed to retrieve the target node by name from the graph prior to a forward or backward pass. Please note that this search can be relatively expensive for large graphs.
